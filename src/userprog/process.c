...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  /*+ ZIYI CHEN
      Block parent process by calling sema_down to wait 
      for the exit of child process. */
...
process_exit (void)
{
  ...
  /*+ ZIYI CHEN
      Iterate over the file list of this process,
      file_close each of them, and file_allow_write ()
      is called inside file_close (). Free these file
      pointers. */
  ...
  /*+ ZIYI CHEN
      Reenable file write on the executable file of the proc. */
  if (cur->exec != NULL) {
  ...
  /*+ ZIYI CHEN
      sema_up sys_wait_sema to nofity its waiting parent process. */
  if (cur->cp->wait) {
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  if (success) {
    /*+ ZIYI CHEN
      cur->exec is the source file, the executable file
      of this process. It denies writes until the proc
      exits.

      Why choose to add it here? 
      
      We have to make sure that thread_current () is 
      getting the correct thread, the thread that executes 
      the executable file.

      This load () is called in start_process (), which is 
      the thread function. 

      Therefore, we know that it is the right thread.
      */
