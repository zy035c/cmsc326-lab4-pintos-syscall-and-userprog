...
syscall_handler (struct intr_frame *f) // UNUSED) 
{
 ...
 case SYS_CREATE:
  ...
  // read from file, call s_read()
  case SYS_READ:
    get_args(f->esp, (void **)&s_args, 3);

    /*Nm starte here*/
    int fd_r = (int)(s_args[0]);
...
ZIYI DONE HERE */
  ...
  case SYS_WAIT:
    get_args(f->esp,(void **)&s_args,1);
    tid_t child_process = (tid_t)(s_args[0]);
    f->eax = s_wait (child_process); 
    break;
...
int user_to_kernel_ptr(const void *vaddr)
{
  ...
  void *ptr = pagedir_get_page(pd, vaddr);
  if (!ptr)
    {
      s_exit(ERROR);
    }
...
struct child_process* add_child_process (int pid)
{
  ...
  cp->pid = pid;
  cp->load = NOT_LOADED;
  cp->wait = false; /*+ This means the proc doesn't wait. */

  /*+ ZIYI CHEN
      Init as 0, since parent proc can't finish wait until
      child proc exits. */
